...
class SRSolver(BaseSolver):
    ...
    def save_checkpoint(self, epoch, is_best ,saving_param):
        """
        save checkpoint to experimental dir
        """
        filename = os.path.join(self.checkpoint_dir, f"Cam{saving_param}_ckp.pth")
        ...
        if is_best:
            print('===> Saving best checkpoint to [%s] ...]' % filename.replace(f"Cam{saving_param}_ckp",
                                                                                f"Cam{saving_param}_best_ckp"))
            torch.save(ckp, filename.replace(f"Cam{saving_param}_ckp",f"Cam{saving_param}_best_ckp"))

        if epoch % self.train_opt['save_ckp_step'] == 0:
            print('===> Saving checkpoint [%d] to [%s] ...]' % (epoch,filename.replace(f"Cam{saving_param}_ckp",
                                                                                       f"Cam{saving_param}_best_ckp"%epoch)))

            torch.save(ckp, filename.replace(f"Cam{saving_param}_ckp",f"Cam{saving_param}_best_ckp"%epoch))
